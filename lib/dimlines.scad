...
function find_fraction(fraction) = (
    (fraction[0] == 0)
        ? fraction
        :
    (fraction[0]/2 == floor(fraction[0]/2))
        ? find_fraction([fraction[0]/2, fraction[1]/2])
        : fraction
    );
...
function inches_part(length) =
    str(
        (floor(remove_feet(length)/units_per_inch) == 0)
            ? ""
            : floor(remove_feet(length)/units_per_inch),
        ((floor(remove_feet(length)/units_per_inch) == 0 || fractional_inches(remove_feet(length))[0] == 0)
            ? ""
            : " "
        ),
        (fractional_inches(remove_feet(length))[0] == 0)
            ? ""
            : str(
                fractional_inches(remove_feet(length))[0],
                "/",
                fractional_inches(remove_feet(length))[1])
    );

function feet_part(length) =
    ((floor((length*10/units_per_inch/12)) == 0)
        ? ""
        : floor((length*10/units_per_inch/12))
    );

function convert_to_inches(length) =
    str(
        feet_part(length),
        ((feet_part(length) == "")
            ? ""
        ...
        ((feet_part(length) == "" || inches_part(length) == "")
            ? ""
            : " "
        ),
        inches_part(length),
